 ...
 * Connects to existing client ETimeTrack database and fingerprint device
 * Real-time sync between client's ETimeTrack system and GymTight Fitness Admin Panel
 */
...
class ETimeTrackBridge extends EventEmitter {
  constructor() {
    super();
    this.dbPath = ''; // Path to client's existing ETimeTrack database
    ...
    this.watchedTables = ['CHECKINOUT', 'USERINFO', 'DEPARTMENTS']; // Standard ETimeTrack tables
    this.simulationMode = false; // Client has real hardware - start in real mode
  }
  ...
  async initializeFirebase() {
    try {
      ...
      if (firebase) {
        this.db = firebase.db;
        console.log('âœ… Firebase Admin initialized via shared config');
      } else {
        console.error('âŒ Failed to initialize Firebase');
      }
    } catch (error) {
  ...
  /**
   * Configure connection to client's existing ETimeTrack database
   */
  configure(config) {
    this.dbPath = config.dbPath || process.env.ETIMETRACK_DB_PATH || '';
    this.connectionString = config.connectionString || process.env.ETIMETRACK_CONNECTION_STRING || `Provider=Microsoft.ACE.OLEDB.12.0;Data Source=${this.dbPath}`;
    this.syncInterval = config.syncInterval || parseInt(process.env.SYNC_INTERVAL_MS) || 5000;
  }
  ...
  async testConnection() {
    try {
      console.log('ðŸ” Testing connection to ETimeTrack database...');
      
      // Check if database file exists
      if (!this.dbPath || !fs.existsSync(this.dbPath)) {
        console.log('âŒ Database file not found at:', this.dbPath);
        return false;
      }
      ...
      console.log('âœ… ETimeTrack database connection successful');
      console.log('ðŸ“Š Available tables:', await this.getTableNames());
      
      return true;
    } catch (error) {
      console.error('âŒ ETimeTrack connection test failed:', error);
      return false;
    }
  }
  ...
  async readNewFingerprintRecords() {
    try {
      ...
      for (const query of queries) {
        try {
          ...
          if (result && result.length > 0) {
            console.log(`ðŸ“¥ Found ${result.length} new records in ETimeTrack`);
      ...
      console.error('âŒ Error reading fingerprint records from ETimeTrack:', error);
      return [];
    }
  }
  ...
  async syncToFirebase(records) {
    try {
      if (!this.db) {
        console.error('âŒ Firebase not initialized');
        return;
      }
      ...
      for (const record of records) {
        ...
        const attendanceData = {
          ...
          scanTime: new Date(record.punchTime),
      ...
      await batch.commit();
      console.log(`âœ… Synced ${records.length} fingerprint records to Firebase`);
      
      // Emit event for real-time updates
      this.emit('dataSynced', records);
      
    } catch (error) {
      console.error('âŒ Error syncing to Firebase:', error);
    }
  }
  ...
  async startRealTimeMonitoring() {
    if (this.isRunning) {
      console.log('âš ï¸  Real-time monitoring is already running');
      return;
    }

    console.log('ðŸš€ Starting ETimeTrack real-time fingerprint monitoring...');
    
    if (!await this.testConnection()) {
      console.error('âŒ Cannot start monitoring - database connection failed');
      return;
    }
    ...
    const monitorLoop = async () => {
      try {
        ...
        if (newRecords.length > 0) {
          console.log(`ðŸ” Found ${newRecords.length} new fingerprint records`);
        ...
        console.error('âŒ Error in monitoring loop:', error);
    ...
    console.log(`âœ… Real-time monitoring started (checking every ${this.syncInterval}ms)`);
  }
  ...
  stopSync() {
    this.isRunning = false;
    console.log('ðŸ›‘ ETimeTrack monitoring stopped');
  }
  ...
  async manualSync() {
    ...
    const records = await this.readNewFingerprintRecords();
    if (records.length > 0) {
      await this.syncToFirebase(records);
    } else {
      console.log('ðŸ“­ No new records found');
    }
  }
...
// Export singleton instance for ETimeTrack integration
...
module.exports = {
  ...
  getInstance: () => {
    if (!bridgeInstance) {
      bridgeInstance = new ETimeTrackBridge();
    }
